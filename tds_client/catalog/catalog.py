...
class ServiceSpec(object):
    ...
    def services_from_xml(root_element, base_url, namespace):
        ...
        for service_element in root_element.iterfind(xml.namespaced_name('service', namespace)):
            ...
            children = ServiceSpec.services_from_xml(service_element, base_url, namespace)
...
class Catalog(CatalogEntity):
    ...
    def get_child_catalogs(self, force_reload=False):
        if force_reload or not self._child_catalogs:  # (self._child_catalogs is None):
            ...
            for catalog_ref_xml in xml.search(self.as_xml_tree(force_reload), 'catalogref', 'dataset'):
                catalog_url = xml.get_attr(catalog_ref_xml, 'href', xml.XLINK)
                if catalog_url is not None:
                    self._child_catalogs.append(Catalog(urls.resolve_path(self.url, '..', catalog_url), self._client))
    ...
    def get_services(self, type_, force_reload=False):
        type_ = type_.lower()
        return [s for s in self._get_services(force_reload).values() if s.type.lower() == type_]

    def get_datasets(self, force_reload=False):
        if force_reload or (self._datasets is None):
            self._datasets = {}

            for dataset_xml in xml.search(self.as_xml_tree(force_reload), 'dataset', 'dataset'):
    ...
    def _get_services(self, force_reload):
        if force_reload or (self._services is None):
            ...
            self._services = ServiceSpec.services_from_xml(catalog_xml, self.url, namespace)
